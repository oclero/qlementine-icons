...
CPP_MAP_BEGIN_MARKER = "  // ---MAP - This part is generated by scripting. Don't modify."
CPP_MAP_END_MARKER = '  // ---MAP'
...
def modify_cpp_file(map_lines: list[str], cpp_filepath: str) -> None:
  ...
  begin_index = cpp_content.find(CPP_MAP_BEGIN_MARKER)
  if begin_index == -1:
    print(f'Cannot find map begin marker in: "{cpp_filepath}"')
    sys.exit(1)

  map_start_index = cpp_content.find('\n', begin_index)
  if map_start_index == -1:
    print(f'Cannot find map content start in: "{cpp_filepath}"')
    sys.exit(1)

  end_index = cpp_content.find(CPP_MAP_END_MARKER, map_start_index)
  if end_index == -1:
    print(f'Cannot find map end marker in: "{cpp_filepath}"')
    sys.exit(1)

  map_content = '\n  '.join(map_lines)
  new_cpp_content = cpp_content[:map_start_index + 1] + \
      f'  {map_content}\n' + cpp_content[end_index:]
